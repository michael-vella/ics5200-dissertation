...
class DGLBondsTrainTestStrategy(TrainTestStrategy):
    ...
    def get_train_test(
        ...
        self._logger.info("Finished train-test sets retrieval for the DGL (Bonds) feature type")
    ...
    def _build_assay_dict(
        ...
        for assay in assays:
            ...
            task_df.columns = ["y", "mol"]  # y is the target, mol is the embedding (DGL Bonds)

            for index, r in task_df.iterrows():
                if r.mol.edata['edge_feats'].shape[-1] < 17:
                    self._logger.info(f"Dropping molecule '{r.mol}'")
                    task_df.drop(index, inplace=True)
